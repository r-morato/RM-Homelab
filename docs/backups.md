...
container**, gzipped, written atomically to a path in the container's own filesystem - so it rides along in the same archive and also stays on the running guest as a ~0.5 MB
...
Backup results are pushed to a **phone-push webhook** (the sole notification channel - the email path was removed because it was undeliverable from a residential IP and caused
